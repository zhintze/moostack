...
- Immersive Aircraft with Create textures baked in + Create recipes (mechanical crafting for aircraft)
...
| `immersive_aircraft-1.4.1+1.21.1-createtextures.jar` | Create-themed textures baked in: all aircraft, engines, propellers, hulls, and accessories reskinned to match Create's industrial aesthetic. No resource pack required. |
...
## Blood Magic (1.21.1 Port)

Blood Magic 3 ported from 1.20.1 to NeoForge 1.21.1. This is an MVP release with core systems functional and some features deferred to v2.

### Included Features (Fully Functional)

**Core Systems:**
- Blood Altar (all 5 tiers with data-driven structure validation)
- Soul Network and LP (Life Points) system
- Sacrificial Knife and Dagger of Sacrifice
- Alchemy Table with full recipe support
- Alchemy Arrays (arcane ash patterns)
- Hellfire Forge (Soul Forge crafting)

**Demon Will System:**
- Raw, Corrosive, Destructive, Vengeful, Steadfast will types
- Tartaric Gems (petty, lesser, common, greater, grand)
- Soul Snares for will collection
- Demon Crucible and Crystallizer
- Will Catalysts for crystal growth acceleration
- Demon Pylon for aura distribution

**Sentient Equipment:**
- Sentient Sword, Pickaxe, Axe, Shovel, Scythe
- All 5 will aspects affect tool behavior
- Epic Fight compatibility (Sentient Scythe uses Tachi style)

**Rituals (50+):**
- Ritual Diviner (Dawn and Dusk tiers)
- Ritual Tinkerer for will-based augmentation
- Well of Suffering (LP automation)
- Condor (creative flight)
- Meteor summoning
- Crystal harvesting and splitting
- Living armor evolution
- All ritual multiblocks render correctly

**Living Armor System:**
- Full upgrade progression (100-300 points)
- Training Bracelet for upgrade management
- Upgrade Tomes for applying/removing upgrades
- All downgrades functional (with penalties)

**Alchemy Flask Potions:** (DISABLED)
- Flask system not functional in 1.21.1
- All flask recipes disabled until system is fixed

**Dungeon System:**
- Simple dungeon access (Edge of the Hidden Realm)
- Standard dungeon access (Pathway to the Endless Realm)
- Tau Fruit cultivation
- Demonite ore and processing
- Dungeon keys and structures

**Altar Runes:**
- All basic runes (Speed, Sacrifice, Self-Sacrifice, Capacity, etc.)
- Enhanced runes (Tier 2 variants)
- Acceleration and Charging runes

**Sigils (20+):**
- Divination, Seer, Water, Lava, Air, Void
- Green Grove, Fast Miner, Magnetism
- Suppression, Haste, Whirlwind, Phantom Bridge
- Teleposition, Transposition, Bounce, Frost
- Blood Light, Ender Severance, Claw, Holding

### Excluded Features (Planned for v2)

These features were present in 1.20.1 but are not yet ported:

**Item Routing System:**
- Routing Nodes (input, output, master)
- All filter types (standard, mod, tag, composite, enchantment)
- Node Router tool
- Master routing node upgrades

**Explosive Charges:**
- Shaped Charge (all tiers)
- Vein Mine Charge (all tiers)
- Deforester Charge (all tiers)
- Fungal Charge (all tiers)
...
**Recipes Missing:**
- Flask crafting at Blood Altar (use Alchemy Table recipes instead)

### Patchouli Guide Status

The in-game guide book requires Patchouli mod and is mostly functional:
- Core progression documented
- Most recipes display correctly
- Multiblock previews work for altars and rituals
- Some entries may reference unavailable items (locked with v2_feature_locked advancement)

Known issues in the guide:
- Some recipe pages may show missing items for v2 features
- A few images may be missing or incorrectly sized
- Cross-references to excluded features are placeholder text

### Configuration

No special configuration required. Blood Magic integrates automatically with:
- JEI (recipe viewing)
- Curios (baubles/trinkets)
...
Note: Create Immersive Aircraft textures are now baked directly into the mod (see Custom Mod JARs section).
